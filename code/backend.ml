...
(* The layout for this version of the backend is slightly more complex
   than we saw earlier.  It consists of
     - uid_loc a function that maps LL UidSet to their target x86 locations
...
(* Compiles a parallel move instruction into a sequence of moves, pushing and
   ...
   move the incoming arguments to their destination UidSet/registers.
...
(* no layout ---------------------------------------------------------------- *)
(* This register allocation strategy puts all UidSet into stack
...
(* greedy layout ------------------------------------------------------------ *)
(* This example register allocation strategy puts the first few UidSet in
...
(* TASK: Implement a (correct) register allocation strategy that
   ...
   algorithm that uses some simple preference heuristics (e.g., for
   call arguments) should be able to beat the greedy algorithm.
...
(* Feel free to ignore or modify the definitions given above. *)
...
let ppi m =
  Llutil.mapcat "\n" (fun (u, s) ->
      Printf.sprintf "%s: %s" u (UidSet.to_string s)) (UidMap.bindings m)

let pppre m =
  Llutil.mapcat "\n" (fun (u, s) ->
      Printf.sprintf "%s: %s" u (Alloc.str_loc s)) (UidMap.bindings m)

let pppref m =
  Llutil.mapcat "\n" (fun (u, s) ->
      Printf.sprintf "%s: %s" u (str_locset s)) (UidMap.bindings m)
...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  let init_ifg = 
    fold_fdecl 
      (fun acc_g (u, _) -> UidMap.add u UidSet.empty acc_g)
      (fun acc_g _ -> acc_g)
      (fun acc_g (u, insn) -> 
        if insn_assigns insn 
        then UidMap.add u UidSet.empty acc_g 
        else acc_g  
      )
      (fun acc_g _ -> acc_g)
    UidMap.empty f in 
  let ifg: UidSet.t UidMap.t =
    fold_fdecl
      (fun acc_g _ -> acc_g)
      (fun acc_g _ -> acc_g)
      (fun acc_g (uid, _) ->
        let liveness_set = live.live_out uid in
        UidSet.fold (fun temp_uid old_map ->
  ...
  let precolored_nodes: Alloc.loc UidMap.t =
    fold_fdecl
      (fun acc_g (uid, _) -> UidMap.add uid (alloc_arg()) acc_g)
      (fun acc_g _ -> acc_g)
      (fun acc_g _ -> acc_g)
      (fun acc_g _ -> acc_g)
    UidMap.empty f
  ...
  let preferred_locations: LocSet.t UidMap.t =  
        ...
        begin match i with
          | Call (_, _, args) ->
            ...
            List.fold_left (
              ...
              begin match op with
                | Id id ->
                  if idx >= 6 then acc else
                  let prev_preference = match UidMap.find_opt id acc with
                    | None -> LocSet.empty
                    | Some x -> x
                  in
                  UidMap.add id (LocSet.add (arg_loc idx) prev_preference) acc
                | _ -> acc
              end
            ) acc_g new_args
          | _ -> acc_g
    ...
    empty.preference f
  in
  let rec colour_graph (k: int) (g: graph) : Alloc.loc UidMap.t  =
    let {interference;precolored;preference} = g in 
    let non_precolored_nodes = UidMap.filter (fun uid _ -> not @@ UidMap.exists (fun key _ -> key = uid) precolored) interference in
    if UidMap.is_empty non_precolored_nodes 
      then precolored
    else
      let less_than_k = UidMap.filter (fun _ neighbours -> UidSet.cardinal neighbours < k) non_precolored_nodes in
      let nodes = if UidMap.is_empty less_than_k then non_precolored_nodes else less_than_k in 
      let (c_node, c_neigh) =
  ...
  let uid_to_loc = colour_graph k {interference=ifg;precolored=precolored_nodes;preference=preferred_locations} in
  let lo =
    fold_fdecl
      (fun lo (x, _) -> (x, UidMap.find x uid_to_loc)::lo)
      (fun lo l -> (l, Alloc.LLbl (Platform.mangle l))::lo)
      (fun lo (x, i) ->
        if insn_assigns i
        then (x, UidMap.find x uid_to_loc)::lo
        else (x, Alloc.LVoid)::lo)
      (fun lo _ -> lo)
      [] f in
  { uid_loc = (fun x -> List.assoc x lo)
